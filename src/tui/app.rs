...
impl App {
    ...
    fn ui(&mut self, f: &mut Frame) {
        ...
        if !self.number_input.is_empty() || self.input_mode == InputMode::Search { footer_height += 1; } // Input line
    ...
    fn render_word_list(&mut self, f: &mut Frame, area: Rect) {
        ...
        let (visible_start, visible_end) = self.get_visible_list_bounds();
    ...
    fn get_visible_list_bounds(&self) -> (usize, usize) {
        // Get the actual visible range that matches what the list widget shows
    ...
    fn render_chart(&self, f: &mut Frame, area: Rect) {
        ...
        let selected_fit_ratio = if self.selected_index < self.word_counts.len() {
            let selected_word = &self.word_counts[self.selected_index];
    ...
    fn render_footer(&self, f: &mut Frame, area: Rect) {
        let mut lines = vec![
            Line::from("Navigation: j/k | g/G/[num]g | Ctrl+u/d/b/f | Chart: L(log) Z(zipf) A(scope) | /(search) n/N | q(quit)")
        ];
